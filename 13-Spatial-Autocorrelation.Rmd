...
lisa <- local_moran(queen_w, ma["rnd"], permutations = 9999)
plot_clust(poly=ma, rnd = "rnd",lisa=lisa,  p= TRUE)
```
...
We still have quite a few $I_i$ values associated with a very low pseudo p-value (The smallest p-value in this example is `r min(lisa$p_vals)`), This would lead one to assume that several polygons exhibit significant levels of spatial autocorrelation with their neighbors when in fact this is due to chance since we did not explicitly generate localized positive or negative spatial autocorrelation. 
...
Using the previous realization of a random process that generated a minimum pseudo p-value of `r min(lisa$p_vals)`, the FDR technique would have generated a reference p-value of `r round(lisa_fdr(lisa, 0.05),5)`--a value smaller than the smallest computed pseudo p-value of `r min(lisa$p_vals)`. Adopting this correction to our output would have resulted in *no* local $I_i$ values being associated with a significant level at or below 0.05 when corrected for the multiple comparison problem.

It's important to note that there is no one perfect solution to the multiple comparison problem. Add to this the other challenges facing inferential interpretation in a spatial framework that includes the potential for overlapping neighbors (i.e. same polygons used in separate permutation tests), the traditional approaches to interpreting  significance becomes challenging. It is thus recommended to interpret these tests with caution.
