...
```{r echo = FALSE, fig.width = 6, fig.height=1.5, fig.cap="Upper left-hand corner of elevation raster"}
# Plot the upper left hand corner
zoom1 <- ext(-180, -179, 89,90)
OP <- par(pty="s",  mfrow=c(1,3), mar=c(0,0,3,0), bty="n")
plot(crop(elevation, zoom1), colNA="bisque", asp=1, axes=FALSE, legend=FALSE,
...
This exercise demonstrates how to use functions from the `gdistance` package to generate a cumulative distance raster. This package does not yet recognize `terra` raster opbjects. As such, we will also need to load the `raster` libray to convert `terra` raster objects to a format recognized by `gdistance` functions. 
```{r}
library(gdistance)
library(raster)
```
