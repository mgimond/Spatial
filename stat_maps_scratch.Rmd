...
The previous chapter demonstrated how the choice of classitification schemes can generate different *looking* maps. This chapter will focus on statisical approaches to generating classification breaks. 

In the field of data analysis, much emphasia is placed in characterizing a dataset via is measure of centrality (e.g. the mean and the median) and its measure of spread (e.g. standard deviational units) with the goal of understanding the nature of the distribution such as it shape (e.g. symmetrical, skewed, etc...) and range. For example, 
...
p2<- ggplot(ma, aes(house_inc)) + geom_histogram(bins = 10) + scale_x_continuous()
grid.arrange(p1,p2, ncol = 2, height = 3)
```

# Mapping stastical distributions
